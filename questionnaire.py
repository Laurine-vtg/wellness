...
# Initialisation Supabase (les variables d'environnement doivent être définies)
SUPABASE_URL = os.getenv("SUPABASE_URL")
SUPABASE_KEY = os.getenv("SUPABASE_KEY")
supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)

# --- Fonctions utilisant Supabase ---

# Fonction pour enregistrer réponse au questionnaire (upsert)
def save_response(data):
    try:
        response = supabase.table("questionnaire").upsert({
            "Nom": data["Nom"],
            "Club": data["Club"],
            "Date": data["Date"],
            ...
            "DescriptionDouleurs": data["Description des douleurs"]
        }).execute()
        if response.error:
...
def load_responses(nom=None, club=None, date=None):
    try:
        ...
        if nom:
            query = query.eq("nom", nom)  # adapte ici en fonction de ta base (nom vs Nom)
        if club:
...
def supprimer_reponse(nom, date):
    try:
        response = supabase.table("questionnaire") \
            .delete() \
            .eq("Nom", nom) \
            .eq("Date", date) \
            .execute()
...
def save_preferences(nom, prefs):
    try:
        data = {"Nom": nom}
        data.update(prefs)
        response = supabase.table("preferences").upsert(data).execute()
        if response.error:
            return False, response.error.message
        return True, ""
    except Exception as e:
...
def save_preferences_2(nom, mode_questionnaire):
    try:
        response = supabase.table("preferences").upsert({
            "Nom": nom,
            "mode_questionnaire": mode_questionnaire
        }).execute()
        if response.error:
            return False, response.error.message
        return True, ""
