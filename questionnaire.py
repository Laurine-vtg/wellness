...
conn = sqlite3.connect("questionnaire_data.db", check_same_thread=False)
c = conn.cursor()

# Création de la table questionnaire (si elle n'existe pas encore). C'est ici qu'on va stocker les réponses au questionnaire.
c.execute('''
CREATE TABLE IF NOT EXISTS questionnaire (
    ...
    Dynamisme INTEGER,
    Douleurs TEXT,
    DescriptionDouleurs TEXT,
    UNIQUE(Nom, Club, Date) ON CONFLICT REPLACE
)
''')
conn.commit()

# Création de la table de préférences utilisateurs (si elle n'existe pas encore). 
c.execute('''
CREATE TABLE IF NOT EXISTS preferences (
    Nom TEXT PRIMARY KEY,
    show_seance INTEGER DEFAULT 1,
    show_weekly_intensity INTEGER DEFAULT 1,
    show_weekly_parameter INTEGER DEFAULT 1,
    show_weekly_score_bien INTEGER DEFAULT 1,
    show_weekly_comp INTEGER DEFAULT 1,
    show_monthly_intensity INTEGER DEFAULT 1,
    show_monthly_parameter INTEGER DEFAULT 1,
    show_monthly_score_bien INTEGER DEFAULT 1,
    show_monthly_comp INTEGER DEFAULT 1,
    show_monthly_zscore INTEGER DEFAULT 1,
    show_global_intensity INTEGER DEFAULT 1,
    show_global_parameter INTEGER DEFAULT 1,
    show_global_score_bien INTEGER DEFAULT 1,
    show_global_zscore INTEGER DEFAULT 1,
    show_seance_coach INTEGER DEFAULT 1,
    show_weekly_intensity_coach INTEGER DEFAULT 1,
    show_weekly_parameter_coach INTEGER DEFAULT 1,
    show_weekly_score_bien_coach INTEGER DEFAULT 1,
    show_weekly_comp_coach INTEGER DEFAULT 1,
    show_monthly_intensity_coach INTEGER DEFAULT 1,
    show_monthly_parameter_coach INTEGER DEFAULT 1,
    show_monthly_score_bien_coach INTEGER DEFAULT 1,
    show_monthly_comp_coach INTEGER DEFAULT 1,
    show_monthly_zscore_coach INTEGER DEFAULT 1,
    show_global_intensity_coach INTEGER DEFAULT 1,
    show_global_parameter_coach INTEGER DEFAULT 1,
    show_global_score_bien_coach INTEGER DEFAULT 1,
    show_global_zscore_coach INTEGER DEFAULT 1,
    mode_questionnaire TEXT DEFAULT 'Tous les jours',
    show_team_intensity_coach INTEGER DEFAULT 1,
    show_cadran INTEGER DEFAULT 1,
    show_team_bien_etre_coach INTEGER DEFAULT 1,
    show_team_douleurs_coach INTEGER DEFAULT 1,
    show_team_synthèse_intensity_coach INTEGER DEFAULT 1,
    show_cadran_synthèse INTEGER DEFAULT 1,
    show_team_synthèse_bien_etre_coach INTEGER DEFAULT 1,
    show_seance_team_coach INTEGER DEFAULT 1
)
''')
conn.commit()

# Fonction pour enregistrer réponse au questionnaire (crée nouvelle ligne ou remplace ligne existante si joueur fait modification pour une même date).
def save_response(data):
    try:
        c.execute('''
        INSERT INTO questionnaire (Nom, Club, Date, Intensite, Sommeil, Fatigue, Stress, Dynamisme, Douleurs, DescriptionDouleurs)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(Nom, Club, Date) DO UPDATE SET
            Intensite=excluded.Intensite,
            Sommeil=excluded.Sommeil,
            Fatigue=excluded.Fatigue,
            Stress=excluded.Stress,
            Dynamisme=excluded.Dynamisme,
            Douleurs=excluded.Douleurs,
            DescriptionDouleurs=excluded.DescriptionDouleurs
        ''', (
            data["Nom"],
            data["Club"],
            data["Date"],
...
# Fonction pour récupérer réponses au questionnaire stockées dans la base de données (avec des filtres facultatifs : nom, club, date).
def load_responses(nom=None, club=None, date=None):
    query = "SELECT * FROM questionnaire WHERE 1=1"
    params = []
    if nom:
        query += " AND Nom = ?"
        params.append(nom)
...
# Nom du fichier SQLite enregistrer sur ordinateur.
DB_PATH = "questionnaire_data.db"  

# Fonction pour supprimer une ligne dans la table questionnaire.
def supprimer_reponse(nom, date):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM questionnaire WHERE Nom = ? AND Date = ?", (nom, date))
    conn.commit()
    conn.close()

# Fonction pour charger les utilisateurs depuis CSV et les convertir en dictionnaire utilisable dans l'application. 
...
# Variable qui stocke les différents utilisateurs.
...
def get_preferences(nom):
    c.execute("SELECT * FROM preferences WHERE Nom = ?", (nom,))
    row = c.fetchone()
    if not row:
        ...
        columns = [desc[0] for desc in c.description][1:]  # Skip Nom
        return dict(zip(columns, row[1:]))

# Fonction pour enregistrer ou mettre à jour les préférences d'affichage graphique.
def save_preferences(nom, prefs):
    values = [int(prefs[k]) for k in prefs]
    placeholders = ", ".join([f"{k} = ?" for k in prefs])
    c.execute(f'''
        INSERT INTO preferences (Nom, {", ".join(prefs.keys())})
        VALUES (?, {', '.join(['?'] * len(prefs))})
        ON CONFLICT(Nom) DO UPDATE SET {placeholders}
    ''', [nom] + values + values)
    conn.commit()

def save_preferences_2(nom, mode_questionnaire):
    c.execute('''
        INSERT INTO preferences (Nom, mode_questionnaire)
        VALUES (?, ?)
        ON CONFLICT(Nom) DO UPDATE SET mode_questionnaire = excluded.mode_questionnaire
    ''', (nom, mode_questionnaire))
    conn.commit()

# Fonction pour déterminer les couleurs d'arrière plan sur certains graphiques ou tableaux.
...
# Fonction pour appliquer un style de couleur aux tableaux contenant les valeurs moyennes.
...
# Fonction pour voir quel joueur n'a pas répondu au questionnaire pour telle date.
...
# Fonction pour associer coach aux bons joueurs
def get_coach_for_club(club_name):
    df_ids = pd.read_csv("ID.csv", sep=";")  # adapte le séparateur si nécessaire
    ...
    return None
# Dictionnaire qui attribue des accès différents selon le rôle de l'utilisateur.
ACCESS_RIGHTS = {
    "coach": [
